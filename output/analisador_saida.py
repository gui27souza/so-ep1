...
trocas, instrucoes, quantuns = [], [], []

for caminho in arquivos:
    t, i, q = extrair_metricas(caminho)
    print(f"{os.path.basename(caminho)} -> Trocas={t}, Instruções={i}, Quantum={q}")
    if t is not None and i is not None and q is not None:
        trocas.append(t)
        instrucoes.append(i)
        quantuns.append(q)
